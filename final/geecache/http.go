...
type HTTPPool struct{
	...
	peers *consitenthash.Map
	httpsGetters map[string]*httpGetter //keyed by e.g. "http://10.0.0.1:8008", 映射远程节点和对应的httpGetter
}
...
func (h *httpGetter) Get(group string, key string) ([]byte, error){
	u := fmt.Sprintf(
		"%v%v/%v",
		h.baseURL,  //将要访问的远程节点的地址
		url.QueryEscape(group),
		url.QueryEscape(key),
	)
	res, err := http.Get(u) //http.Get方法获取返回值，并转换为[]byte类型
	if err != nil{
		return nil, err
	}
